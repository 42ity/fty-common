...
default|default-Werror|default-with-docs|valgrind|clang-format-check)
    ...
    # Start of recipe for dependency: fty-log
    if ! (command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list libfty_log-dev >/dev/null 2>&1) || \
           (command -v brew >/dev/null 2>&1 && brew ls --versions fty-log >/dev/null 2>&1) \
    ; then
        echo ""
        BASE_PWD=${PWD}
        echo "`date`: INFO: Building prerequisite 'fty-log' from Git repository..." >&2
        $CI_TIME git clone --quiet --depth 1 https://github.com/42ity/fty-log fty-log
        cd fty-log
        CCACHE_BASEDIR=${PWD}
        export CCACHE_BASEDIR
        git --no-pager log --oneline -n1
